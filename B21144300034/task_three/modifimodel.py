...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        sin_input=np.float32(cur_line[0:20])
        cos_output = np.float32(cur_line[20])
        for i in range(time_step-1):
          cur_line = self.lines[index+1+i].split(',')
          sin_input = np.vstack((sin_input,cur_line[0:20]))
          cos_output = np.vstack((cos_output,cur_line[20]))
...
class Rnn(nn.Module):
    def __init__(self, input_num=20, hidden_num=32, layer_num=2, output_num=1, seq_len=1000):
...
optimizer = torch.optim.SGD(rnn.parameters(), lr=0.02, momentum=0.8)
...
if __name__ == '__main__':
    ...
    for i in range(total_epoch):  # 模型训练1000轮
        train_rnn(i)
    torch.save(rnn, dir +'O3rnn_model.pkl')
    print("Stop Training!")
...
plt.ylabel('loss')
plt.xlabel('epoch')
plt.savefig(dir +'O3trainloss.jpg')
...
def test_rnn():
    ...
    net_test = torch.load(dir +'O3rnn_model.pkl')  # load model
    hidden_state = None
    test_loss = 0
    net_test.eval()
    with torch.no_grad():
...
if __name__ == '__main__':
    ...
    print(r_v.shape)
    # 对比图3
    data = p_v.reshape(-1,1)
    data2 = r_v.reshape(-1,1)
    plt.plot(data[150:200].cpu(), c='green')
    plt.plot(data2[150:200].cpu(), c='orange')
    plt.xlabel('data')
    plt.yticks(np.arange(0,1,0.05))
    plt.ylabel('prediction')
    plt.savefig(dir +'O3数据训练50次气体检测结果总.jpg')
